...
app = FastAPI(title="Filtered Experimental Viewer", version="1.0")
...
def find_latest_jsonl() -> Optional[Path]:
    """최신 filtered JSONL 파일 찾기"""
    pattern = str(FILTERED_DATA_DIR / "llm_filtered_*.jsonl")
...
def load_data(force_reload: bool = False) -> List[Dict[str, Any]]:
    """Load filtered experimental data from JSONL"""
    ...
    if not jsonl_path or not jsonl_path.exists():
        print(f"No filtered JSONL found in: {FILTERED_DATA_DIR}")
        return []
    ...
    try:
        ...
        _data_cache = data
        print(f"Loaded {len(data)} filtered papers from {jsonl_path.name}")
