...
Goal
----
Decide whether the given paper is primarily about **Rechargeable Aqueous Zinc Metal Batteries** using liquid electrolytes.

*** CRITICAL EXCLUSION RULES (If any apply, answer NO) ***
1. **Zinc-Air Batteries (ZAB):** EXCLUDE. Even if they use aqueous electrolytes, their mechanism (air cathode) is different from ZMB.
2. **Wearable / Flexible / Soft Batteries:** EXCLUDE. Papers focusing on gel polymer electrolytes, hydrogels, or solid-state electrolytes for wearable devices are NOT the target. We focus on **LIQUID** electrolyte systems.
3. **Zinc-Ion Hybrid Capacitors:** EXCLUDE.
4. **Redox Flow Batteries:** EXCLUDE.

Definition (YES criteria)
-------------------------
Answer "YES" ONLY if ALL of the following are true:
- **Anode:** Metallic zinc (Zn foil, Zn plate, Zn powder, or Zn on 3D host).
- **Electrolyte:** LIQUID aqueous electrolyte (salt dissolved in water).
- **Application:** Rechargeable zinc metal batteries (ZMB/ZIB).

Answer "NO" if:
- Main system is Li/Na/K/Mg/Ca/Al batteries.
- Non-aqueous / Organic / Ionic Liquid electrolytes.
- Focus is purely on cathode materials (e.g., MnO2, V2O5) without discussing Zn anode issues.
...
{{
  ...
  "reason": "<Short reason, e.g., 'Rejected: Zinc-Air battery', 'Rejected: Gel electrolyte/Wearable', or 'Accepted: Standard liquid AZMB'>"
}}
"""
...
Goal
----
Determine if the paper reports an **EX-SITU PROTECTIVE LAYER** applied to the Zn anode **BEFORE** cell assembly.

*** CRITICAL CRITERIA (Precision-First) ***

1. [TARGET] Ex-situ Protective Layer (YES)
   - A distinct layer (polymer, inorganic, hybrid, SEI) coated/deposited on the Zn anode surface **BEFORE** battery assembly.
   - **Important:** Includes methods termed "In-situ" or "Self-assembled" in the text, **ONLY IF** the process happens **outside** the full battery cell (e.g., pre-passivation by immersion/dipping, chemical etching, electrodeposition in a separate cell).
   - *Key Logic:* Is the Zn anode modified/coated *before* it meets the actual cathode? -> YES.

2. [EXCLUDE] Pure In-situ SEI (NO)
   - SEI layers formed naturally **during** cycling solely due to **electrolyte additives** or solvent engineering.
   - No physical/chemical pre-treatment on the Zn anode itself.

3. [EXCLUDE] Separator/Interlayer (NO)
   - A freestanding layer, membrane, or separator modification physically placed between anode and cathode.
   - If it is NOT chemically/physically bonded to the Zn surface -> NO.

4. [EXCLUDE] Simple 3D Host / Structure Design (NO)
   - 3D hosts (e.g., CNT, Cu foam, graphene, porous Zn) used MERELY to lower local current density or accommodate volume change.
   - **Exception (YES):** If a 3D host is fabricated AND THEN coated with an **additional** protective layer (e.g., "Nafion-coated 3D Zn").

5. [EXCLUDE] Wearable / Gel Electrolyte (NO)
   - If the paper focuses on "Wearable", "Flexible", "All-solid-state", or "Hydrogel/Gel Polymer" electrolytes -> NO. (Stage 1 should have caught this, but check again).

6. [REQUIREMENT] Lab-Scale Experiments
   - The paper must show experimental results (cycling, plating/stripping tests, full cell performance). Theoretical-only (DFT-only) papers -> NO.

Task
----
Analyze the inputs (Title, Abstract, Method Snippets) to classify the paper.
...
{{
  ...
  "modification_focus": "ZN_EX_SITU_LAYER/ZN_3D_HOST/ZN_IN_SITU_SEI/ELECTROLYTE/SEPARATOR_INTERLAYER/OTHER",
  "confidence": 0.0-1.0,
  "reason": "Brief explanation. e.g. 'Rejected: Wearable gel electrolyte', 'Accepted: Zn foil dipped in solution X before assembly', 'Rejected: 3D host without extra coating'."
}}
...
def parse_xml_metadata(xml_dir: Optional[Path], pii: str) -> Tuple[str, str]:
    ...
    if not xml_path.exists():
        print("XML_PATH가 없습니다.")
        return "", ""
    print(xml_path, "가 있습니다")
    title = ""
    abstract = ""
    try:
...
def run_for_pii(pii: str, args: argparse.Namespace, cache: Dict[str, Dict[str, Any]], cache_path: Path) -> Dict[str, Any]:
    ...
    if xml_abs:
        # abstract_paragraphs는 리스트 형태여야 하므로 리스트로 감싸서 할당
...
def main():
    ...
    logging.info(f"xml_dir: {args.xml_dir or '(none)'}")
