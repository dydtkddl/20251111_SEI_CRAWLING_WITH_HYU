    ...
    for attempt in range(max_retries):
        try:
            if mode == "cli":
                ...
                raw = ollama_generate_http(ollama_url, model, prompt, timeout, temperature, top_p, keep_alive=keep_alive)
                parsed = extract_first_json_object(raw)
                return raw, parsed, ""
...
1) Output MUST be a SINGLE valid JSON object (RFC8259).
2) Do NOT output markdown, code fences, LaTeX (e.g., \boxed{{}}), comments, or any extra text.
3) Use ONLY these exact keys (English only) and ONLY these allowed values:
...
4) Do NOT add any other keys (no extra fields, no non-English keys).
5) confidence MUST be a number between 0 and 1 (e.g., 0.85). Not a string.
6) reason MUST be ONE short sentence (<= 25 words). No newlines.

If you cannot comply perfectly, output this EXACT fallback JSON and nothing else:
{{"has_exsitu_protective_layer":"NO","has_lab_scale_experiments":"NO","modification_focus":"OTHER","confidence":0.0,"reason":"FORMAT_ERROR"}}
========================
...
def cache_key_stage2(title: str, abstract: str, body_structure: str, supp_structure: str, evidence: str, model: str = "") -> str:
    m = (model or "").strip()
    return sha1_text(
        f"{PIPELINE_VERSION}|{PROMPT_VERSION}|STAGE2|MODEL={m}|"
        f"{title}\n{abstract}\n{body_structure}\n{supp_structure}\n{evidence}".strip()
    )
...
def classify_stage2_exsitu_lab(
    ...
    # Stage2 model determination (allow separate model for Stage2)
    stage2_model = (getattr(args, "stage2_model", "") or args.llm_model).strip()
    ...
    ck = cache_key_stage2(title, abstract, body_structure, supp_structure, evidence_snippets, model=stage2_model)
    ...
    # Parse keep_alive for Stage2
    keep_alive = None
    if getattr(args, "stage2_keep_alive", ""):
        ka = str(args.stage2_keep_alive).strip()
        try:
            keep_alive = int(ka)
        except Exception:
            keep_alive = ka
...
# =============================================================================
# Stage2 Job Data Structure + Helpers (for batch/end scheduling)
# =============================================================================
@dataclass
class Stage2Job:
    paper_id: str
...
def write_stage2_job(paper_dir: Path, job: Stage2Job) -> Path:
    p = paper_dir / "stage2_job.json"
    p.write_text(json.dumps(job.__dict__, ensure_ascii=False, indent=2), encoding="utf-8")
    return p

def load_stage2_job(job_path: Path) -> Stage2Job:
    d = json.loads(job_path.read_text(encoding="utf-8"))
    return Stage2Job(**d)
...
def run_for_pii(
    ...
    # If Stage2 is deferred, write a job and return QUEUED (Stage2 will be done in main()).
    if getattr(args, "stage2_schedule", "immediate") in {"batch", "end"}:
        job = Stage2Job(
            paper_id=pii,
        ...
        write_stage2_job(paper_dir, job)

        return {
            "paper_id": pii,
            "status": "QUEUED_STAGE2",
            "xml_title": title,
            ...
            "S2_modification_focus": "NA",
            "S2_confidence": 0.0,
            "S2_reason": f"Deferred Stage2 ({args.stage2_schedule})",
            "candidate_exsitu_lab": "NA",
    ...
    # Otherwise, immediate Stage2 (original behavior)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # Stage2 scheduling / model split
    parser.add_argument(
        "--stage2_schedule",
        choices=["immediate", "batch", "end"],
        default="immediate",
        help="Stage2 execution schedule: immediate(per-paper), batch(run when K queued), end(run all at end).",
    )
    parser.add_argument(
        "--stage2_batch_k",
        type=int,
        default=50,
        help="When stage2_schedule=batch, run Stage2 when queued jobs reach K. (Flush remaining at end.)",
    )
    parser.add_argument(
        "--stage2_model",
        default="",
        help="Model used for Stage2 only. If empty, uses --llm_model.",
    )
    parser.add_argument(
        "--stage2_keep_alive",
        default="",
        help='Optional Ollama keep_alive for Stage2 batch (e.g. "30m", "-1", "0"). Empty=use server default.',
    )
...
# =============================================================================
# Stage2 Batch Execution (for batch/end scheduling)
# =============================================================================
def parse_keep_alive_value(s: str) -> Optional[Any]:
    s = (s or "").strip()
    if not s:
        return None
    try:
...
def run_stage2_batch(
    ...
    ka = parse_keep_alive_value(getattr(args, "stage2_keep_alive", ""))

    # Model swap: unload llm_model (qwen2.5), load stage2_model (qwen3)
    if args.ollama_mode == "http":
        logging.info(f"[Stage2-BATCH] Model swap: unloading {llm_model}, loading {stage2_model}")
        
        # Unload llm_model (qwen2.5) first
        if stage2_model != llm_model:
            try:
                _ = ollama_generate_http(
                    args.ollama_url,
                    llm_model,
                    prompt="{}",
                    timeout=max(10, min(30, args.timeout)),
                    temperature=0.0,
                    top_p=1.0,
                    keep_alive=0,  # unload
                )
        ...
        # Load/warm-up stage2_model (qwen3)
        try:
            _ = ollama_generate_http(
                args.ollama_url,
                stage2_model,
                prompt="{}",
                timeout=max(10, min(60, args.timeout)),
                temperature=0.0,
                top_p=1.0,
                keep_alive=ka if ka is not None else "30m",
            )
        except Exception:
            pass
    ...
    for job in jobs:
        ...
        s2 = classify_stage2_exsitu_lab(
            ...
            body_structure=job.body_structure,
            supp_structure=job.supp_structure,
            evidence_snippets=job.evidence_snippets,
            args=args,
            cache=cache,
            cache_path=cache_path,
            paper_dir=paper_dir,
        ...
        # Update results row in-place
        i = idx_map.get(job.paper_id, None)
        if i is not None:
            r = results[i]
            r["status"] = "DONE"
            r["S2_has_exsitu_protective_layer"] = s2.get("has_exsitu_protective_layer", "NO")
            r["S2_has_lab_scale_experiments"] = s2.get("has_lab_scale_experiments", "NO")
            r["S2_modification_focus"] = s2.get("modification_focus", "OTHER")
            r["S2_confidence"] = s2.get("confidence", 0.0)
            r["S2_reason"] = s2.get("reason", "")

            # Candidate recompute
            cand = "YES" if (
                r.get("S1_is_aqueous_zmb") == "YES"
                and r.get("S2_has_exsitu_protective_layer") == "YES"
                and r.get("S2_has_lab_scale_experiments") == "YES"
            ) else "NO"
            r["candidate_exsitu_lab"] = cand

    # Model swap back: unload stage2_model (qwen3), reload llm_model (qwen2.5)
    if args.ollama_mode == "http" and stage2_model != llm_model:
        logging.info(f"[Stage2-BATCH] Model swap back: unloading {stage2_model}, reloading {llm_model}")
        
        # Unload stage2_model (qwen3)
        try:
            _ = ollama_generate_http(
                args.ollama_url,
                stage2_model,
        ...
        try:
            _ = ollama_generate_http(
                ...
                temperature=0.0,
                top_p=1.0,
                keep_alive="30m",  # keep loaded for next papers
            )
        except Exception:
            pass
...
def main():
    ...
    logging.info(f"Stage2 schedule={args.stage2_schedule} batch_k={args.stage2_batch_k} model={args.stage2_model or args.llm_model}")

    results: List[Dict[str, Any]] = []
    failures: List[Dict[str, Any]] = []

    # Stage2 queue for batch/end mode
    stage2_queue: List[Stage2Job] = []
    idx_map: Dict[str, int] = {}  # paper_id -> index in results
    ...
    for pii in tqdm(pii_list, desc="Processing PIIs", unit="paper"):
        try:
            ...
            results.append(r)
            idx_map[pii] = len(results) - 1

            # Queue Stage2 job if deferred
            if r.get("status") == "QUEUED_STAGE2":
                job_path = Path(args.out_dir) / pii / "stage2_job.json"
                if job_path.exists():
                    stage2_queue.append(load_stage2_job(job_path))

                # If batch mode: flush when K reached
                if args.stage2_schedule == "batch" and args.stage2_batch_k > 0:
                    if len(stage2_queue) >= args.stage2_batch_k:
                        logging.info(f"[Stage2-BATCH] Flushing {len(stage2_queue)} jobs (K={args.stage2_batch_k})")
                        run_stage2_batch(stage2_queue, args, cache, cache_path, results, idx_map)
                        stage2_queue = []
    ...
    # Final Stage2 flush (batch/end mode)
    if args.stage2_schedule in {"batch", "end"} and stage2_queue:
        logging.info(f"[Stage2-FINAL] Flushing remaining {len(stage2_queue)} jobs (schedule={args.stage2_schedule})")
        run_stage2_batch(stage2_queue, args, cache, cache_path, results, idx_map)
        stage2_queue = []
