#!/usr/bin/env python3
# jdftx_trj_orthohombic.py
# Orthorhombic lattice 전용 trajectory 추출기
import sys, re, pathlib

if len(sys.argv) != 5:
    print("Usage: python jdftx_trj_orthohombic.py <jdftx.out> <a_bohr> <b_bohr> <c_bohr>")
    sys.exit(1)
...
# Bohr to Angstrom 변환 상수
BOHR_TO_ANG = 0.52917721092

a_bohr = float(sys.argv[2])
b_bohr = float(sys.argv[3])
c_bohr = float(sys.argv[4])

a = a_bohr * BOHR_TO_ANG
b = b_bohr * BOHR_TO_ANG
c = c_bohr * BOHR_TO_ANG

# Orthorhombic lattice basis vectors (in Å)
# a1 = (a, 0, 0)
# a2 = (0, b, 0)
# a3 = (0, 0, c)
lattice = [
...
# Regex pattern for ionic position lines\pattern = re.compile(r"^\s*ion\s+(\w+)\s+([\-\d.eE]+)\s+([\-\d.eE]+)\s+([\-\d.eE]+)\s+\d+\s*$")
...
xyz_path = out_path.with_name("trj_ortho.xyz")
