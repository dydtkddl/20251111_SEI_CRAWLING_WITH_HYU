...
# ====== <<< YOU REQUESTED THIS LINK TO BE USED >>> ======
# Template URL you provided — we'll replace startPage=<n> in this string for each page.
URL_TEMPLATE = ("https://chemistry-europe.onlinelibrary.wiley.com/action/doSearch?"
                "Ppub=&PubType=journal&field1=Keyword&field2=AllField&field3=AllField"
                "&publication=&startPage={page}&text1=Solid+Electrolyte+Interphase&text2=&text3=&pageSize=20")
...
def fetch_range_persistent(profile_dir, out_dir, start, end, headful=False, proxy=None, cookies_path=DEFAULT_COOKIES,
    ...
    with sync_playwright() as p:
        ...
        for idx in tqdm(range(start, end+1), desc="Pages"):
            # Use provided URL template and replace {page} with current page index
            url = URL_TEMPLATE.format(page=idx)
